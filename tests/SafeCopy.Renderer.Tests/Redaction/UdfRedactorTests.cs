...
public class UdfRedactorTests
{
    ...
    public void Redact_OutputCanBeReIngested()
    {
        ...
        try
        {
            ...
            services.AddSingleton<IDocumentSecurityValidator, DocumentSecurityValidator>();
            services.AddSingleton<IFileSystem, FileSystem>();
            services.AddSingleton<SafeCopy.DocumentEngine.Ingestion.IDocumentIngestor, SafeCopy.DocumentEngine.Ingestion.Udf.UdfDocumentIngestor>();
            services.AddSingleton<SafeCopy.Core.Abstractions.IDocumentEngine, SafeCopy.DocumentEngine.Ingestion.DocumentEngine>();
            var sp = services.BuildServiceProvider();
            var engine = sp.GetRequiredService<SafeCopy.Core.Abstractions.IDocumentEngine>();
    ...
    public void Redact_VerificationResidual_ShouldBeZero()
    {
        ...
        try
        {
            ...
            var sp = services.BuildServiceProvider();
            var engine = sp.GetRequiredService<SafeCopy.Core.Abstractions.IDocumentEngine>();
            var detEngine = sp.GetRequiredService<SafeCopy.Core.Abstractions.IDetectionEngine>();
