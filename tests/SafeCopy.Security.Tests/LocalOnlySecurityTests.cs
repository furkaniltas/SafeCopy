...
namespace SafeCopy.Security.Tests;
...
public class LocalOnlySecurityTests
{
    private static string FindRepoRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir.FullName, "SafeCopy.slnx")))
                return dir.FullName;
        ...
        throw new InvalidOperationException("Could not locate repo root (SafeCopy.slnx not found). Searched from " + AppContext.BaseDirectory);
    }
    ...
    public void TempCleanup_CrashCleanup_StaleRemovesOldAndKeepsRecent()
    {
        ...
        var rootBase = Path.Combine(Path.GetTempPath(), "SafeCopy");
