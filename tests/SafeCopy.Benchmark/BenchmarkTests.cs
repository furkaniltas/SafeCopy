using System.Text;
using SafeCopy.Benchmark.Matching;
using SafeCopy.Benchmark.Metrics;
using SafeCopy.Benchmark.Models;
using SafeCopy.Benchmark.Parser;
using FluentAssertions;
using Xunit;

namespace SafeCopy.Benchmark.Tests;
...
public class BenchmarkTests
{
    ...
    public void LabelMapping_DirectAndMapped()
    {
        ...
        LabelMappingTable.Resolve("vkn").SafeCopyType.Should().Be(SafeCopy.Core.Models.DetectionType.TaxId);
    }
    ...
    {
        var rec = new BenchmarkRecord { Id = "1", Text = "Ahmet Yılmaz", Entities = new List<GroundTruthEntity> { new() { Label = "full_name", Start = 0, End = 12, Text = "Ahmet Yılmaz", MappedType = SafeCopy.Core.Models.DetectionType.FullName, MappingStatus = MappingStatus.DIRECT } } };
        var det = new BenchmarkDetection { Type = SafeCopy.Core.Models.DetectionType.FullName, Value = "Ahmet Yılmaz", Start = 0, End = 12, Text = "Ahmet Yılmaz" };
    ...
    {
        var rec = new BenchmarkRecord { Id = "1", Text = "Ahmet Yılmaz", Entities = new List<GroundTruthEntity> { new() { Label = "full_name", Start = 0, End = 12, Text = "Ahmet Yılmaz", MappedType = SafeCopy.Core.Models.DetectionType.FullName, MappingStatus = MappingStatus.DIRECT } } };
        var det = new BenchmarkDetection { Type = SafeCopy.Core.Models.DetectionType.FullName, Value = "Ahmet", Start = 0, End = 5, Text = "Ahmet" };
        var matches = SpanMatcher.MatchRecord(rec, new List<BenchmarkDetection> { det });
    ...
    public void OneToOneMatching_Deterministic()
    {
        var rec = new BenchmarkRecord
        {
            ...
            {
                new() { Label = "full_name", Start = 0, End = 12, Text = "Ahmet Yılmaz", MappedType = SafeCopy.Core.Models.DetectionType.FullName, MappingStatus = MappingStatus.DIRECT },
                new() { Label = "full_name", Start = 13, End = 24, Text = "Mehmet Kaya", MappedType = SafeCopy.Core.Models.DetectionType.FullName, MappingStatus = MappingStatus.DIRECT }
            }
        };
        var dets = new List<BenchmarkDetection>
        {
            new() { Type = SafeCopy.Core.Models.DetectionType.FullName, Value = "Ahmet Yılmaz", Start = 0, End = 12, Text = "Ahmet Yılmaz" },
            new() { Type = SafeCopy.Core.Models.DetectionType.FullName, Value = "Ahmet Yılmaz", Start = 0, End = 12, Text = "Ahmet Yılmaz" } // duplicate
        };
    ...
    public void FpCalculation()
    {
        ...
        var det = new BenchmarkDetection { Type = SafeCopy.Core.Models.DetectionType.Phone, Value = "0532", Start = 0, End = 4, Text = "0532" };
        var matches = SpanMatcher.MatchRecord(rec, new List<BenchmarkDetection> { det });
