...
namespace SafeCopy.Benchmark.Runner;
...
public sealed class BenchmarkRunner
{
    ...
    public static string LocateDataset(string? preferredPath = null)
    {
        ...
        while (dir != null)
        {
            ...
            candidates.Add(Path.Combine(dir.FullName, "src", "SafeCopy.App", "Resources", "tr_privacy_tr_curated.jsonl"));
            dir = dir.Parent;
            if (dir == null || dir.FullName.Length < 3) break;
        }
        // Fallback to existing test.jsonl
        candidates.Add(Path.Combine("src", "SafeCopy.App", "Resources", "test.jsonl"));
        candidates.Add("D:\\SafeCopy\\src\\SafeCopy.App\\Resources\\test.jsonl");
    ...
    private static string? TryGetEmbeddedDataset()
    {
        ...
        try
        {
            ...
            var appAsm = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name == "SafeCopy.App");
            if (appAsm != null)
            {
    ...
    private List<BenchmarkDetection> DetectText(string text)
    {
        ...
        var doc = new Document { Name = "benchmark.txt", Format = global::SafeCopy.Core.Abstractions.DocumentFormat.Txt, Pages = new[] { page }, Metadata = new DocumentMetadata() };
    ...
    public static string GenerateMarkdown(BenchmarkResult r)
    {
        ...
        sb.AppendLine("# SafeCopy Turkish PII Benchmark Report");
