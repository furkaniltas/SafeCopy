...
public class DatasetResourceTests
{
    ...
    public void TestJsonl_EmbeddedResource_ExistsInAppAssembly()
    {
        ...
        names.Should().Contain("SafeCopy.App.Resources.test.jsonl");
    }
    ...
    public void TestJsonl_SinglePhysicalCopy_UsedByBothProjects()
    {
        ...
        appAssembly.GetManifestResourceNames().Should().Contain("SafeCopy.App.Resources.test.jsonl");
        ...
        testAssembly.GetManifestResourceNames().Should().NotContain("SafeCopy.App.Tests.Resources.test.jsonl");
        ...
        while (dir != null && !File.Exists(Path.Combine(dir.FullName, "SafeCopy.slnx")))
            dir = dir.Parent;
        if (dir != null)
        {
            var singleCopy = Path.Combine(dir.FullName, "src", "SafeCopy.App", "Resources", "test.jsonl");
            File.Exists(singleCopy).Should().BeTrue("Single physical copy must exist at src/SafeCopy.App/Resources/test.jsonl");
        }
    }
}
