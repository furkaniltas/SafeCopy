...
namespace EksimSafeCopy.Ocr.Tests;
...
public class OcrEngineTests
{
    ...
    {
        var engine = new LocalOcrEngine();
        engine.EngineName.Should().Contain("EksimSafeCopy.LocalOcr");
    }
    ...
    public void ImageIngestion_WithOcr_ReturnsOcrText()
    {
        ...
        services.AddSingleton<EksimSafeCopy.DocumentEngine.Ingestion.IDocumentIngestor>(sp =>
        ...
        var ingestor = provider.GetServices<EksimSafeCopy.DocumentEngine.Ingestion.IDocumentIngestor>().First();
        ...
        try
        {
            var result = ingestor.Ingest(tempFile, new EksimSafeCopy.DocumentEngine.Ingestion.IngestionOptions());
            result.IsSuccess.Should().BeTrue();
