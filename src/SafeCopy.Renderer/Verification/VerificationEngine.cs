namespace SafeCopy.Renderer.Verification;
...
public sealed class VerificationEngine : SafeCopy.Core.Abstractions.IVerificationEngine
{
        ...
        try
        {
            ...
            if (isPartial && originalDetections.Count > 0)
            {
                ...
                foreach (var d in detections.Where(d => d.State != DetectionState.FalsePositive && d.State != DetectionState.Deselected))
                {
                    ...
                    {
                        var expectedMasked = SafeCopy.Renderer.Redaction.PartialMaskingPolicy.Mask(orig.Type, orig.Value);
                    ...
                    else
                    {
                        ...
                        {
                            var masked = SafeCopy.Renderer.Redaction.PartialMaskingPolicy.Mask(o.Type, o.Value);
...
private Result<(Document Document, IDetectionEngine DetectionEngine)> LoadDocumentForVerification(string filePath, DocumentFormat format, CancellationToken cancellationToken)
    {
        ...
        services.AddSingleton<IFileSystem, SafeCopy.Infrastructure.FileSystem>();
        ...
        services.AddSingleton<IIbanDetector, SafeCopy.Detectors.Detection.Detectors.IbanDetector>();
        services.AddSingleton<IXlsxStructuredDetector, SafeCopy.Detectors.Detection.Detectors.XlsxStructuredDetector>();
