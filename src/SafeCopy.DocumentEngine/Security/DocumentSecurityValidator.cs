namespace SafeCopy.DocumentEngine.Security;
...
public sealed class DocumentSecurityValidator : IDocumentSecurityValidator
{
    ...
    public Result ValidateFileAccess(string filePath)
    {
        ...
        try
        {
            ...
            // Allow unknown extensions if signature detection can identify the format
            if (!_options.AllowedExtensions.Contains(extension))
            {
                var detectedFormat = DetectFormatFromSignature(filePath);
                if (detectedFormat == DocumentFormat.Unknown)
                {
                    // Try ZIP-based format identification
                    var zipFormat = IdentifyZipBasedFormat(filePath);
    ...
    {
        var detectedFormat = DetectFormatFromSignature(filePath);
        // For formats without reliable signature detection (TXT, ZIP-based formats),
        // allow Unknown as valid if extension matches expected format
        var noSignatureFormats = new HashSet<DocumentFormat>
        {
            DocumentFormat.Txt,
            DocumentFormat.Docx,
            DocumentFormat.Xlsx,
        ...
        if (detectedFormat == DocumentFormat.Unknown)
        {
            if (noSignatureFormats.Contains(expectedFormat))
                return Result.Success();
    ...
    public DocumentFormat DetectFormatFromStream(Stream stream)
    {
        ...
        try
        {
            ...
            {
                // ZIP-based formats need deeper inspection to differentiate
                return DocumentFormat.Unknown;
            }
            // PNG: \x89PNG\r\n\x1a\n
    ...
    /// <summary>
    /// Performs deep inspection of ZIP-based formats to identify specific format.
    /// Used by DocumentEngine.DetectFormat for format identification.
    /// </summary>
    public DocumentFormat IdentifyZipBasedFormat(string filePath)
    {
        try
        {
            using var stream = File.Open(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete);
    ...
    /// <summary>
    /// Performs deep inspection of ZIP-based formats from stream to identify specific format.
    /// </summary>
    public DocumentFormat IdentifyZipBasedFormatFromStream(Stream stream)
    {
        var originalPosition = stream.Position;
        try
        {
            ...
            // Check for DOCX: word/document.xml
            if (archive.GetEntry("word/document.xml") != null)
                return DocumentFormat.Docx;
            // Check for XLSX: xl/workbook.xml
            if (archive.GetEntry("xl/workbook.xml") != null)
                return DocumentFormat.Xlsx;
            // Check for UDF: content.xml (UYAP document format)
            if (archive.GetEntry("content.xml") != null)
...
public interface IDocumentSecurityValidator
{
    ...
    DocumentFormat IdentifyZipBasedFormat(string filePath);
    DocumentFormat IdentifyZipBasedFormatFromStream(Stream stream);
